...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < num_recv; i++)
        {
            ...
            uint32_t h = murmur3((const void *)ft, sizeof(ft), 0x11111111) % 5;
            if (h <= 1)
            ...
            if (update_cm)
            {
                for (int i = 0; i < M; i++)
                {
                    uint32_t j = murmur3((const void *)ft, sizeof(ft), i * i) % N;
                    uint32_t newval = counters[i][j] + 1;
            ...
            else
            {
                ...
                // std::map<struct fivetuple, uint32_t>::iterator it = fastpath_htbl.find(*key);
                memset(&l, 0, sizeof(record_t));
                l.key = *ft;
                HASH_FIND(hh, records, &l.key, sizeof(Fivetuple), p);
                if (p)
                {
                    p->cnt += 1;
                }
                ...
                else
                {
                    ...
                    r->key = *ft;
                    r->cnt = 0;
                    HASH_ADD(hh, records, key, sizeof(Fivetuple), r);
                }
            }
