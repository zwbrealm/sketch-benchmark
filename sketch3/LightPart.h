...
public:
    ...
    /* insertion */
    void insert(const void *key, int f = 1)
    {
        // uint32_t hash_val = (uint32_t)bobhash->run((const char *)key, KEY_LENGTH_4);
        uint32_t hash_val = MurmurHash3_x86_32(key, sizeof(key), 0x12345678);
    ...
    {
        uint32_t hash_val = MurmurHash3_x86_32((const char *)key, sizeof(key), 0x12345678);
        uint32_t pos = hash_val % (uint32_t)counter_num;
