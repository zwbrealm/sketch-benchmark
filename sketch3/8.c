...
fivetuple2 *ft2 = (fivetuple2 *)malloc(sizeof(fivetuple2));
...
FlowRadar fr = FlowRadar(10485760, 3, 3, 0.1);
SketchVisor skv = SketchVisor(5, 10000, 10485760);
UnivMon um = UnivMon(873812);
Nitrosketch ns = Nitrosketch(327680, 0.5);
...
void got_packet(u_char *argv, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    memset(ft2, 0, sizeof(fivetuple2));
    ft->saddr = ip->saddr;
    ft->daddr = ip->daddr;
    ft->sport = udp->sport;
    ft->dport = udp->dport;
